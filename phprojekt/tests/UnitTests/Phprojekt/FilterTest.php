...
class Phprojekt_FilterTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testAddFilterPart1()
    {
        ...
        $this->assertEquals("( (`title` LIKE '%root%' ) AND (`title` LIKE '%root1%' )  )", $filter->getWhere());

        $filter->addFilter('title', 'like', 'root2', 'OR');
        $this->assertEquals("( (`title` LIKE '%root%' ) AND (`title` LIKE '%root1%' ) OR (`title` LIKE '%root2%' )  )",
            $filter->getWhere());
    }
    ...
    public function testAddFilterPart2()
    {
        ...
        $filter->addFilter('title', 'like', 'root', 'AND');
        $this->assertEquals("(projectId = 1) AND ( (`title` LIKE '%root%' )  )", $filter->getWhere());

        $filter->addFilter('title', 'like', 'root1', 'AND');
        $this->assertEquals("(projectId = 1) AND ( (`title` LIKE '%root%' ) AND (`title` LIKE '%root1%' )  )",
            $filter->getWhere());

        $filter->addFilter('title', 'like', 'root2', 'OR');
        $this->assertEquals("(projectId = 1) AND ( (`title` LIKE '%root%' ) AND (`title` LIKE '%root1%' )"
            . " OR (`title` LIKE '%root2%' )  )", $filter->getWhere());
    }
    ...
    public function testConvertRulePart1()
    {
        ...
        $this->assertEquals("( (DATE(`meeting_datetime`) = '2010-05-12' )  )", $filter->getWhere());

        $filter = new Phprojekt_Filter($item);
        $filter->addFilter('meetingDatetime', 'equal', '12:00', 'AND');
        $this->assertEquals("( (TIME(`meeting_datetime`) = '10:00:00' )  )", $filter->getWhere());

        $filter = new Phprojekt_Filter($item);
        $filter->addFilter('meetingDatetime', 'equal', '1273665600', 'AND');
        $this->assertEquals("( (`meeting_datetime` = '2010-05-12 10:00:00' )  )", $filter->getWhere());

        $filter = new Phprojekt_Filter($item);
        $filter->addFilter('endTime', 'equal', '12:00', 'AND');
        $this->assertEquals("( (`end_time` = '10:00:00' )  )", $filter->getWhere());
    }
