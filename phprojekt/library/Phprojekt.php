...
class Phprojekt
{
    ...
    public static function errorHandler($errNumber, $errStr, $errFile, $errLine)
    {
        ...
        if ($useLog) {
            $messageLog  = sprintf("%s\n File: %s - Line: %d\n Description: %s\n", $errDesc, $errFile, $errLine, $errStr);

            // The frames always contain the file and line where the function was called
            // To get a format like "2: File:line class->function" we print the file and line of the first frame (which is
            // this function, so we don't need the function part anyways) and then print class->function\nFile:line for
            // each other frame. The last frame will be the php script that has been called, so it's fine that we don't
