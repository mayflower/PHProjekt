...
class Phprojekt_User_Information extends Phprojekt_ModelInformation_Default
{
    ...
    public function setFields()
    {
        ...
        // firstname
        $this->fillField('firstname', 'First name', 'text', 2, 5, array(
            'required' => true,
            'length'   => 255));

        // lastname
        $this->fillField('lastname', 'Last name', 'text', 3, 6, array(
            'required' => true,
            'length'   => 255));

        // status
        $this->fillField('status', 'Status', 'selectbox', 4, 10, array(
            'range'    => array($this->getFullRangeValues('A', 'Active'),
                                $this->getFullRangeValues('I', 'Inactive')),
            'default'  => 'A'));

        // admin
        $this->fillField('admin', 'Admin', 'selectbox', 5, 11, array(
