...
class Timecard_Models_Timeproj extends Phprojekt_ActiveRecord_Abstract implements Phprojekt_Model_Interface
{
    ...
    public function __construct($db = null)
    {
        ...
        $this->_informationManager = Phprojekt_Loader::getModel('Timecard', 'TimeprojInformation');
    }
    ...
    public function recordValidate()
    {
        ...
        $amount = (int) $amount;
        if (($amount > 2400) || ($amount < 30)) {
            $this->_validate->error->addError(array(
                'field'   => 'Amount',
                'label'   => Phprojekt::getInstance()->translate('Amount'),
                'message' => Phprojekt::getInstance()->translate('The amount is invalid (from 30 to 2400)')));
                return false;
        }
    ...
    public function getRecords($date)
    {
        ...
        $numRows = count($datas['timecard']) + count($datas['timeproj']);
